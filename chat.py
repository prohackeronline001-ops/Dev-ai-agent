...
# Loaded from a local .env file when running locally; on Railway these come
# from the Variables tab, so load_dotenv() simply finds nothing and no-ops.
load_dotenv()
...
if not TELEGRAM_BOT_TOKEN or not HF_API_TOKEN:
    sys.exit(
        "Missing required environment variables. Set TELEGRAM_BOT_TOKEN and "
        "HF_API_TOKEN (locally via a .env file, or in Railway's Variables tab)."
    )
