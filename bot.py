...
def clear_webhook() -> None:
    """Make sure no webhook is set, and drop any pending update queue so a
    fresh instance doesn't collide with a stale one on startup."""
    try:
        ...
        if response.status_code == 200:
            ...
            logger.warning(f"deleteWebhook returned status {response.status_code}: {response.text}")
    except Exception as e:
...
def main() -> None:
    """
    Runs the bot with a simple retry loop instead of recursive self-calls.
    Recursion here previously risked stacking multiple polling loops in the
    same process (or leaving a half-torn-down one behind), which is exactly
    what produces Telegram's 409 Conflict: 'terminated by other getUpdates
    request'. A loop that fully rebuilds and re-enters run_polling() avoids
    that, since run_polling() already blocks until it's told to stop and
    cleans up after itself before we build the next Application.
    """
    clear_webhook()
    ...
    while True:
        ...
        try:
            # close_loop=False keeps asyncio well-behaved across retries
            application.run_polling(drop_pending_updates=True, close_loop=False)
            break  # run_polling() only returns normally on a clean shutdown
        except Exception as e:
